...
Собран на **Astro 5** без UI-фреймворка: интерактива на странице пять штук, и все
пять закрываются платформой (`<dialog>`, `scroll-snap`, пара обработчиков) — React
здесь не окупается.
...
| `src/data/catalog.ts` | торты, десерты, фото зала и производства, отзывы, шаги заказа |
...
| `src/layouts/BaseLayout.astro` | мета, OpenGraph, JSON-LD, шрифт, лайтбокс |
...
- **Фотографии — плейсхолдеры Unsplash.** Хранится только id снимка, `srcset`
  собирает `Photo.astro` из URL-параметров. Когда приедут реальные фото — положить в
  `src/assets/` и переключить компонент на `astro:assets`.
- **Карта — виджет Яндекса по `oid` организации**, с табами между точками. Параметр
  `pt` (свои метки) виджетом игнорируется, поэтому карточка организации: в ней сразу
  рейтинг, часы и кнопка «как добраться». Iframe грузится по `IntersectionObserver`,
  чтобы не тормозить первую загрузку.
- **Лайтбокс — нативный `<dialog>`:** Esc, ловушка фокуса и возврат фокуса достаются
  от платформы. Кликабельно всё фото, а не маленькая кнопка в углу.
...
- [ ] **Текст «О нас»** — сейчас заготовка на публично подтверждаемых фактах
...
- [ ] **`public/og.jpg`** — картинка для соцсетей, 1200×630
