...
- [ ] **Шаг 6:** коммит `feat(directus): инстанс и схема контента`.
...
- [ ] **Шаг 1:** написать скрипт. Логика по шагам, каждый ищет объект по имени
...
- [ ] **Шаг 2:** запустить локально с админ-токеном, сохранить токен сборщика
  в `.env` проекта (`DIRECTUS_TOKEN`). Повторить запуск: вывод только
  «setup: готово».
- [ ] **Шаг 3:** проверить под «Редактором» в браузере: видит 8 коллекций,
  может создать афишу, не может добавить направление.
- [ ] **Шаг 4:** коммит `feat(directus): роли, сборщик и пересборка скриптом`.
...
- [ ] **Шаг 1:** написать и запустить одноразовый скрипт переноса; в Directus
  появляется весь контент, картинки видны в медиатеке.
- [ ] **Шаг 2:** `scripts/export-fixture.mjs`:
...
- [ ] **Шаг 3:** `pnpm fixture` (после переезда на pnpm в 2.1 — до этого
  `node scripts/export-fixture.mjs`) с `DIRECTUS_TOKEN` сборщика: появляется
  `fixture.json` и папка файлов.
- [ ] **Шаг 4:** `.gitignore` += `src/data/fixture-files/`, `public/media/`;
  битая строка комментария заменяется на «# скриншоты проверок в корне».
- [ ] **Шаг 5:** коммит `feat(content): перенос данных в Directus и фикстура`.
...
- [ ] **Шаг 1:** тесты в `select.check.ts`: окно с пустыми границами, граница
...
- [ ] **Шаг 2:** запуск → падение на импорте.
- [ ] **Шаг 3:** реализация чистыми функциями без зависимостей, сравнение дат
  строками `YYYY-MM-DD`.
- [ ] **Шаг 4:** зелёный.
- [ ] **Шаг 5:** коммит `feat(content): выборки афиши, акций и меню`.
...
- [ ] **Шаг 1:** разметка: `<dialog id="book" aria-labelledby="book-title">`,
...
- [ ] **Шаг 3:** стили: панель снизу на мобильном (`inset-block-end: 0`,
...
- [ ] **Шаг 4:** проверка клавиатурой: Tab по трём кнопкам, Escape закрывает,
  фокус возвращается на триггер. Скриншоты.
- [ ] **Шаг 5:** коммит `feat(booking): диалог брони через мессенджеры`.
...
- [ ] **Шаг 1:** `nav.ts`: `navFor(content)` → `[Главная #top, Афиша #afisha
...
- [ ] **Шаг 2:** `StickyBar.astro`: две кнопки «Позвонить» и «Заказать
...
- [ ] **Шаг 3:** мобильное меню: фокус-ловушка не нужна, но `body` получает
  `overflow: hidden` при открытом меню.
- [ ] **Шаг 4:** скриншоты 390 (меню закрыто/открыто, полоса видна после
...
- [ ] **Шаг 1:** сетка `#about`: заголовок `home.about_title`, лид
  ...
  условий в разметке.
- [ ] **Шаг 2:** на 390 — колонка, большие карточки с фото сверху; на 1440 —
  2 + 3. Шапка карточек по паттерну `.card-top`.
- [ ] **Шаг 3:** скриншоты, коммит `feat(directions): пять направлений компании`.
...
- [ ] **Шаг 1:** секция рендерится только при `content.afisha`. Десктоп: две
  ...
  текст под ним.
- [ ] **Шаг 2:** дата по-русски через `Intl.DateTimeFormat('ru-RU', { day:
  'numeric', month: 'long' })`, `<time datetime>`.
- [ ] **Шаг 3:** скриншоты, коммит `feat(afisha): секция афиши`.

### Задача 4.2: Акции

**Файлы:** `Promos.astro`.

- [ ] **Шаг 1:** `livePromos` + `groupByPlace`; группы: «Обе точки» (null),
...
- [ ] **Шаг 2:** сетка 1 / 2 / 3 колонки, карточки одинаковой высоты, кнопка
  прижата к низу.
- [ ] **Шаг 3:** скриншоты, коммит `feat(promos): акции по адресам`.

### Задача 4.3: Меню

**Файлы:** `Menus.astro`.

- [ ] **Шаг 1:** три карточки по направлениям `cafe`, `restaurant`,
...
- [ ] **Шаг 2:** скриншоты, коммит `feat(menu): меню трёх направлений`.
...
- [ ] **Шаг 1:** секция `#delivery` на оливковой панели по образцу карточки
...
- [ ] **Шаг 2:** скриншоты, коммит `feat(delivery): приложение и магазин с мокапом`.
...
- [ ] **Шаг 1:** воспроизвести: клик «Показать на карте» второй точки → через
  секунду виджет откатывается на первую (`scrollIntoView` будит наблюдатель).
- [ ] **Шаг 2:** в обработчике клика `observer?.disconnect()`; в `mount()`
...
- [ ] **Шаг 3:** руками в браузере: переключение точек и вкладок отзывов
...
- [ ] **Шаг 1:** `PRODUCT.md`: «Что это» — визитка компании с пятью
