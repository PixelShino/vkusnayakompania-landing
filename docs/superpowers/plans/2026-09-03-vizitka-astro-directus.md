...
- [x] **Шаг 1:** фото `home.hero_image` на весь экран (мобильный —
...
- [x] **Шаг 2:** LCP: `getImage` preload в `BaseLayout` берёт `sizes` из одной
  константы `HERO_SIZES` в `src/lib/hero.ts`, а не дублирует.
- [x] **Шаг 3:** контраст текста на фото ≥ 4,5:1 за счёт градиента; проверить
  на светлом фото.
- [x] **Шаг 4:** скриншоты, коммит `feat(hero): первый экран визитки`.
...
- [x] **Шаг 1:** упростить: без фильтров, сетка 2 / 3 / 4 колонки, фото 3:4 с
...
- [x] **Шаг 2:** скриншоты, коммит `feat(cakes): витрина работ без цен`.
...
- [x] **Шаг 1:** `PRODUCT.md`: «Что это» — визитка компании с пятью
  ...
  Карт, VPS, SMTP.
- [x] **Шаг 2:** `DESIGN.md`: новые паттерны — диалог, постер, карточка акции,
  лента галереи, нижняя полоса, мокап телефона; раздел «Движение» дополнить
  константами из CLAUDE.md.
- [x] **Шаг 3:** `README.md`: команды pnpm, переменные окружения, локальный
  Directus, фикстура, деплой на VPS, Vercel как превью, ссылка на
  `docs/DIRECTUS.md`; Windows-предупреждение оставить.
- [x] **Шаг 4:** коммит `docs: документация визитки и Directus`.

### Задача 7.3: Финальная проверка и отчёт

- [x] `pnpm test && pnpm check && pnpm build` зелёные с Directus и с фикстурой.
- [x] Под ролью «Редактор»: сменить афишу, добавить акцию, заменить PDF,
  добавить фото в галерею — после `pnpm build` всё на странице.
- [x] Все чекбоксы плана отмечены, ветка `feature/vizitka-directus` чистая.
- [x] Отчёт в чат: сделано, решения, отложено, что нужно от клиента;
  дубль в Telegram.
